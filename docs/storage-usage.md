...
書き込みの途中（`ALLOCATING`・`REGISTERING`・`PREPARING`）にあるslotは、走査しても配置の途中経過しか見えないため、その値をそのslotの使用量として公開しない。
実体はroot合計に数えたままにして、準備中の割当量が`Unmanaged`へ移らないようにする。
公開しない結果、再び準備へ入ったslotは前の準備で測った値を次の周期で落とし、`measurement`は`pending`へ戻る。
