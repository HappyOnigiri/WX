...
# hookはツール導入と独立に入れ直せるよう、setupの依存にはせず単独のtargetにする。
setup-hooks:
...
# 診断1件の表示文はmessage IDで持つという規約。IDを書き忘れるとResolveが英語リテラルを残すため、
# 訳さないと決めた文と登録し忘れた文が見分けられなくなる。後者だけをこの検査で落とす。
findings-check:
	$(GO) run ./tools/checkfindings
...
static-check: fmt-check lint deadcode mod-tidy-check docs-check comments-check tests-check lines-check testlayout-check fuzz-check gitexec-check migrations-check catalog-check display-check findings-check automation-check docs-index-check workflow-check reporter-check shell-check

ci-checks: static-check coverage-check ci-test-race build-darwin version-check smoke release-check

# hook本体は`make setup-hooks`で共通Gitディレクトリのhooks直下へ置き、user側のdispatcherを維持する。
# 正本は`scripts/hooks/`にあり、hooksディレクトリの中身は複製である。
# 以下はそのhookが呼び出す契約である。pre-pushのhookは持たず、コミット時のこの検査だけをゲートとする。
hook-pre-commit:
	scripts/hook-check.sh

hook-plan:
	scripts/hook-check.sh --plan
