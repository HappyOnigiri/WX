...
- **会话管理与清理：** `wx sessions`、`wx resume`、`wx gc --dry-run`、`wx clean --dry-run`。
- **立即删除 worktree：** `wx clean` 不等待保留期限即删除未使用的 worktree，`wx clean --all` 会先请求正在使用的会话停止再删除。
  删除前会先保存工作，恢复数据与会话历史都会保留。
