...
- **セッション管理・クリーンアップ:** `wx sessions`、`wx resume`、`wx gc --dry-run`、`wx clean --dry-run`。
- **worktree の即時削除:** `wx clean` は、保持期限を待たずに未使用の worktree を削除します。`wx clean --all` は使用中のセッションにも終了を要求してから削除します。
  削除前に作業は保存し、復元データとセッション履歴は残します。
