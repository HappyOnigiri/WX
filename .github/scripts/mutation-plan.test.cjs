...
test('archive sharding rejects duplicate, missing, and unexpected sources', () => {
  ...
  definitions[1].files[0] = definitions[0].files[0];
  ...
  unexpected[2].files.push('future.go');
...
test('archive matrix uses unique artifacts and excludes tests and other sources', () => {
  ...
  assert.equal(new Set(plan.matrix.map((item) => item.id)).size, 3);
  assert.deepEqual(plan.matrix.map((item) => item.profiles), ['internal/archive', 'internal/archive', 'internal/archive']);
