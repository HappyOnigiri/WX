...
// localization_help.go は help.go の英語 help 本文だけを日本語へ寄せる。
// 訳すのは説明文で、コマンド名・オプション名・path などの機械識別子は原文を保つ。
...
func translateHelp(text string, lang i18n.Language) string {
	...
	// 段の適用順は訳出の契約である。長い段落を持つ remainder を先に当て、
	// 残った行を短い語句の replacements で補うため、この順序を入れ替えない。
	text = applyTranslations(text, remainder)
	text = applyTranslations(text, replacements)
