...
// writeUsage は英語では render の出力をそのまま通し、日本語では翻訳を挟む。
func TestWriteUsageTranslatesOnlyJapanese(t *testing.T) {
	render := func(w io.Writer) { _, _ = io.WriteString(w, "Usage: wx status\n") }
	var english, japanese bytes.Buffer
	writeUsage(&english, render, i18n.English)
	writeUsage(&japanese, render, i18n.Japanese)
	if english.String() != "Usage: wx status\n" {
		t.Fatalf("English usage=%q", english.String())
	}
	if japanese.String() != "使い方: wx status\n" {
		t.Fatalf("Japanese usage=%q", japanese.String())
	}
}
