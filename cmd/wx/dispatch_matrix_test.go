...
func TestCommandDispatchRejectsMalformedAndUnavailableRequests(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "clear extra argument", args: []string{"clear", "one", "two"}, want: 2},
		{name: "clear replenish without a mode", args: []string{"clear", "--replenish"}, want: 2},
		{name: "clear unavailable", args: []string{"clear"}, want: 1},
		{name: "clear workspace unavailable", args: []string{"clear", "/tmp/workspace"}, want: 1},
