...
func runSetup(ctx context.Context, args []string) int {
	...
	if languageUnset, loadErr := setupLanguageUnset(); loadErr != nil {
		_, _ = fmt.Fprintln(session.errOut, i18n.T(ctx, "common.error", nil)+":", loadErr)
		return 1
		...
		if selectErr != nil {
			...
			_, _ = fmt.Fprintln(session.errOut, i18n.T(ctx, "common.error", nil)+":", selectErr)
			return 1
		}
...
func selectSetupLanguage(ctx context.Context, session setupSession) (i18n.Language, error) {
	...
	step := setup.Step{ID: "language", Title: loc.Localize("setup.display_language.title", nil), Detail: loc.Localize("setup.display_language.detail", nil), Options: []setup.Action{setup.Action(i18n.English), setup.Action(i18n.Japanese)}, Default: setup.Action(i18n.English)}
...
func runSetupItem(ctx context.Context, options setup.Options, id string, action setup.Action, value string, out, errOut io.Writer) int {
	ctx = commandContext(ctx)
	loc := i18n.New(string(i18n.LanguageFromContext(ctx)))
	errorPrefix := loc.Localize("common.error", nil) + ":"
	step, err := setup.CollectStep(ctx, options, id)
	if err != nil {
		_, _ = fmt.Fprintln(errOut, errorPrefix, err)
		return 1
	}
	if !slices.Contains(step.Options, action) {
		...
		_, _ = fmt.Fprintf(errOut, "%s %s", errorPrefix, loc.Localize("setup.action_unavailable", map[string]any{"Item": id, "Action": string(action)}))
		if len(available) > 0 {
			_, _ = fmt.Fprint(errOut, loc.Localize("setup.available_actions", map[string]any{"Actions": strings.Join(available, ", ")}))
		}
		_, _ = fmt.Fprintln(errOut)
		return 1
	}
	if action == setup.ActionManual && strings.TrimSpace(value) == "" {
		_, _ = fmt.Fprintf(errOut, "%s %s\n", errorPrefix, loc.Localize("setup.manual_requires_value", map[string]any{"Item": id}))
		return 2
	}
	note, err := setup.Apply(ctx, options, step, action, value)
	if err != nil {
		_, _ = fmt.Fprintf(errOut, "%s %s %s: %v\n", errorPrefix, step.ID, action, err)
		return 1
	}
...
func setupOptions() setup.Options {
	return setup.Options{
		...
		RestartDaemon: func(ctx context.Context) error {
			...
			return daemonRestartError(i18n.New(string(i18n.LanguageFromContext(ctx))), failure, err)
		},
	...
	for index, option := range step.Options {
		...
		if step.ID == "language" {
			if option == setup.Action(i18n.English) {
				label = loc.Localize("setup.display_language.english", nil)
			} else if option == setup.Action(i18n.Japanese) {
				label = loc.Localize("setup.display_language.japanese", nil)
			}
		}
...
func runSetupUpdate(ctx context.Context, options setup.Options, session setupSession) int {
	...
	steps, err := setup.Collect(ctx, options)
	if err != nil {
		_, _ = fmt.Fprintln(session.errOut, i18n.T(ctx, "common.error", nil)+":", err)
		return 1
	}
	divergent := setup.Divergent(steps)
	if len(divergent) == 0 {
...
func applySetupStep(ctx context.Context, options setup.Options, session setupSession, step setup.Step) setupOutcome {
	...
	note, err := setup.Apply(ctx, options, step, action, value)
	if err != nil {
		_, _ = fmt.Fprintf(session.errOut, "%s: %s %s: %s\n", i18n.T(ctx, "common.error", nil), step.ID, action, err)
		return setupOutcomeFailed
	}
...
func setupStepValue(session setupSession, step setup.Step) (string, error) {
	...
	prompt := i18n.New(string(localizedUsageLanguage())).Localize("setup.worktree_root_prompt", nil)
	_, _ = fmt.Fprintf(session.errOut, "%s [%s]: ", prompt, step.Desired)
