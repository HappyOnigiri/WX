...
func run(ctx context.Context, args []string) int {
	...
	if agentName == "" {
		if len(f.branches) > 0 || f.fresh {
			fmt.Fprintln(os.Stderr, i18n.T(ctx, "common.error", nil)+":", i18n.T(ctx, "flag.branch_fresh_require_agent", nil))
			topUsageLanguage(os.Stderr, i18n.LanguageFromContext(ctx))
			return 2
		}
		cfg, err := config.Load()
	...
	if agentName != "claude" && agentName != "codex" {
		message := i18n.T(ctx, "flag.unknown_command", map[string]any{"Name": strconv.Quote(agentName)})
