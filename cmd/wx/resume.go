...
func runResume(ctx context.Context, args []string) int {
	...
	if len(rest) > 0 && !strings.HasPrefix(rest[0], "-") && rest[0] != "claude" && rest[0] != "codex" {
		fmt.Fprintln(os.Stderr, i18n.T(ctx, "common.error", nil)+":", i18n.T(ctx, "wx.resume.agent_choice", nil))
		return 2
	}
	...
	if len(*branches) > 0 && !*fresh {
		fmt.Fprintln(os.Stderr, i18n.T(ctx, "common.error", nil)+":", i18n.T(ctx, "wx.resume.branch_needs_fresh", nil))
		return 2
	}
	cfg, err := config.Load()
	if err != nil {
