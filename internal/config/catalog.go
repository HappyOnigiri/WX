...
var catalogTexts = map[string]catalogText{
	...
	"worktree.fetch_default_branch":       {"Fetch the default branch", "Fetches the origin default branch before branch-unspecified worktree leases.", "Keeps new worktrees close to the latest fast-forward origin commit without changing the source checkout.", nil},
	...
	"agent.codex_no_daemon":               {"Codex without shared daemon", "Passes --no-daemon when wx starts Codex.", "When disabled, Codex may use its shared daemon.", nil},
	...
	"update.auto_apply":                   {"Automatic update install", "Lets the daemon install a newer wx by itself once nothing is leased.", "When disabled, a new release is still announced and you install it with wx update --apply.", nil},
	"daemon.login_shell":                  {"Start the daemon through a login shell", "Starts the daemon from the LaunchAgent through your login shell so that its startup files build the environment.", "When disabled, the daemon and the hooks it runs see only the fixed PATH written into the LaunchAgent.", nil},
	...
	"fetch_default_branch":                {"Workspace default-branch fetch", "Fetches the origin default branch before branch-unspecified worktree leases for this workspace.", "A fast-forward remote commit becomes the base of new or updated READY worktrees; explicit --branch leases are unchanged.", nil},
	...
	"onboarding.declined_at":                           {"Initial setup check declined time", "When the user chose not to be asked about the initial worktree setup check again.", "Suppresses the initial setup check prompt for this repository.", nil},
}
