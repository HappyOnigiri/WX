...
// TestReloadConfigDetectsSwappedRootViaCompatibilityHandle exercises the
// identity-comparison path taken for a root that was registered only through
// the legacy rootHandles compatibility map (no descriptor identity recorded
// yet, mirroring a manually constructed or historical Manager). Reload must
// still detect that the directory now underneath that cached handle was
// replaced, instead of skipping validation because no identity was on file.
func TestReloadConfigDetectsSwappedRootViaCompatibilityHandle(t *testing.T) {
	home := t.TempDir()
	t.Setenv("HOME", home)
	store, err := state.Open(filepath.Join(home, "state.db"))
	if err != nil {
	...
	handle, err := os.OpenRoot(worktreeRoot)
	if err != nil {
		t.Fatal(err)
	}
	m := testManager(t, cfg, store)
	...
	m.rootHandles[worktreeRoot] = handle
	m.roots[worktreeRoot] = true
	m.mu.Unlock()
	...
	if err := m.reloadConfig(false); err == nil || !strings.Contains(err.Error(), "inode changed") {
		t.Fatalf("swapped compatibility-handle root reload error=%v", err)
	}
}
