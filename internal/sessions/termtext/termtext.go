// Package termtext は元データに残った端末制御列を表示の直前で無害化し、会話一覧向けの表示整形も持つ。
// 履歴に貼り付けられた ANSI エスケープ列やキャリッジリターンを除去して画面消去や幅計算のずれを防ぎ、
// 相対時刻のように会話一覧だけで使う整形をここに置く（他の画面と規則を共有する整形は internal/textfmt）。
package termtext
