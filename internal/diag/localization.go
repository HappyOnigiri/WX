...
var diagnosticReplacements = []struct{ en, ja string }{
	...
	{"readiness hooks are missing or invalid", "readiness hook が無いか不正です"},
	...
	{"no file of the slot prepared for ", "準備した slot のファイル（"},
	{" was found to share blocks with its main worktree, so the copy took the full size", "）は main worktree と block を共有しておらず、コピーは全サイズになりました"},
