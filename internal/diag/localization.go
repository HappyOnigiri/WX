...
var diagnosticReplacements = []struct{ en, ja string }{
	...
	{"the configuration was loaded, but it contains entries wx does not recognize", "設定は読み込めましたが、wx が解釈しない項目があります"},
	{"wx ignored these entries: ", "wx が解釈しなかった項目: "},
	{"correct the spelling of the reported entries or delete those lines, then run wx config reload", "報告された項目の綴りを直すか該当行を削除し、wx config reload を実行してください"},
