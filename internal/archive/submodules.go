...
// 未保全の理由コード。snapshot が親の HEAD・index・worktree しか保存しないため、子に残る作業の種別を分けて記録する。
const (
...
var unsavedSubmoduleReasonOrder = []string{ReasonModified, ReasonUntracked, ReasonCommitMoved, ReasonMissingObject, ReasonUndetermined}

// UnsavedSubmodule は snapshot が保存できなかった submodule 1 件である。
...
// unsavedSubmodules は親の snapshot に入らなかった子の作業を列挙する。
...
func (m *Manager) unsavedSubmodules(ctx context.Context, repo discovery.Repository, worktree, identity, statusOutput string) []UnsavedSubmodule {
	...
	if err != nil {
		...
		return sortedUnsavedSubmodules(reasons)
	}
