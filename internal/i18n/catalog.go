...
var catalog = map[string]Entry{
	...
	"dashboard.choose_launch":         {EN: "Choose what to run in a borrowed worktree", JA: "borrow した worktree で何を動かすか選ぶ"},
	...
	"menu.claude.label":               {EN: "Launch Claude in a borrowed worktree", JA: "Claude を専用 worktree で起動"},
	...
	"menu.codex.label":                {EN: "Launch Codex in a borrowed worktree", JA: "Codex を専用 worktree で起動"},
	...
	"menu.shell.label":                {EN: "Open a shell in a borrowed worktree", JA: "borrow した worktree でシェルを開く"},
	...
	"menu.run.label":                  {EN: "Run one command in a borrowed worktree", JA: "borrow した worktree でコマンドを 1 つ実行"},
	...
	"menu.release.description":        {EN: "Return a worktree that was borrowed without an agent (by \"borrow a worktree\" or by a hook) and is still held. Enter the session ID that was printed when it was borrowed.", JA: "エージェントを介さずに借りた（「worktree を借りるだけ」や hook による）まま返却されていない worktree を返します。借りたときに表示された session ID を入力してください。"},
	...
	"menu.bench.description":          {EN: "Measure how long this workspace takes to become usable and where that time goes: when an agent can start, when preparation is complete, and how much disk each step uses.", JA: "この workspace が使える状態になるまでの時間と、その内訳を計測します。エージェントが開始できる時点、準備が完了する時点、各段階のディスク使用量が分かります。"},
...
var validationData = map[string]any{
	...
	"Hint": "", "Step": "", "Detail": "", "Directory": "", "Events": "", "Binary": "", "Resolved": "", "Event": "", "Begin": "", "End": "", "Backup": "", "Usage": "", "Query": "", "Name": "", "Command": "", "Timeout": "", "Socket": "", "Guidance": "", "Default": "", "Change": "", "Agent": "",
}
