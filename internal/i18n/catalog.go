...
var catalog = map[string]Entry{
	...
	"dashboard.version":               {EN: "Running wx {{.Version}}", JA: "動作中の wx {{.Version}}"},
	"dashboard.update.available":      {EN: "A newer wx is available: {{.Version}}", JA: "新しい wx があります: {{.Version}}"},
	...
	"menu.update.label":               {EN: "Install the newer wx", JA: "新しい wx を導入する"},
	"menu.update.description":         {EN: "Download the install.sh attached to the latest release and run it. It fetches the binary, checks its checksum and version, replaces the installed wx, and restarts the daemon.", JA: "最新リリースに添付された install.sh を取得して実行します。バイナリを取得し、checksum とバージョンを検証し、インストール済みの wx を置き換えて daemon を再起動します。"},
	"menu.update.impact":              {EN: "This screen is run by the wx that is about to be replaced, so wx exits when the update finishes; start it again to come back.", JA: "この画面は置き換えられる側の wx が動かしているため、更新が終わると wx は終了します。戻るにはもう一度起動してください。"},
