...
func localizeCLIMessage(text string, lang i18n.Language) string {
	...
	for _, replacement := range []struct{ en, ja string }{
		...
		{"--runs must be at least 1", "--runs は1以上で指定してください"},
		...
		{"; run wx bench --reuse to measure without retiring standby worktrees", "。standby worktree を退役させず計測するには wx bench --reuse を実行してください"},
