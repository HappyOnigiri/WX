...
func (s *Store) QuarantineMissingRecoveryRef(ctx context.Context, ref string) error {
	...
	defer tx.Rollback()
	rows, err := tx.QueryContext(ctx, `SELECT DISTINCT session_id FROM snapshots WHERE head_recovery_ref=? OR worktree_recovery_ref=? OR index_recovery_ref=? OR git_state_recovery_ref=? OR conflict_recovery_ref=?`, ref, ref, ref, ref, ref)
	if err != nil {
		return err
	}
	defer rows.Close()
	...
	if _, err := tx.ExecContext(ctx, `UPDATE snapshots SET status='QUARANTINED' WHERE head_recovery_ref=? OR worktree_recovery_ref=? OR index_recovery_ref=? OR git_state_recovery_ref=? OR conflict_recovery_ref=?`, ref, ref, ref, ref, ref); err != nil {
		return err
	}
